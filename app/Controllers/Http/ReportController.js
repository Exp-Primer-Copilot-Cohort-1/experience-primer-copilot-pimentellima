...
const { startOfDay, endOfDay, subHours, format } = require('date-fns')
...
class ReportController {
	async indexHome({ request }) {
		const userLogged = request.qs
		const data = request.only(['date_start', 'date_end', 'prof_id'])

		const startDate = data.date_start
			? startOfDay(new Date(data.date_start))
			: startOfDay(new Date())
		const endDate = data.date_end
		...
		if (!data.prof_id) {
			activities = await Activity.where({
				...
				.where('date', '>=', subHours(startDate, 3))
				.where('date', '<=', endDate)
				.sort('date')
				.fetch()
		} else {
		...
		usersJson.forEach((usr) => {
			...
			if (usr.birth_date) {
				...
				if (
					format(
						new Date(splt[2], parseInt(splt[1], 10) - 1, splt[0]),
						'dd/MM',
		...
		const canceledClient = activitiesJson.filter(
			(ac) => ac.scheduled === 'canceled_client',
		)
	...
	async indexActivities({ request, }) {
		...
		const activitiesJson = activities
			.toJSON()
			.filter((act) => act.scheduled !== 'not_count')
	...
	async indexActivitiesCanceled({ request, }) {
		const userLogged = request.qs
		const data = request.only(['date_start', 'date_end'])
	...
	async indexActivitiesPartner({ request, }) {
		const userLogged = request.qs
		...
		const activitiesJsonData = activities
			.toJSON()
			.filter((act) => act.scheduled !== 'not_count')
		const activitiesJson = activitiesJsonData.filter(
			(act) =>
				act &&
				act.client &&
				act.client.partner &&
				act.client.partner.value &&
				act.client.partner.value === data.partner &&
				new Date(act.date).getTime() >= subHours(startDate, 3).getTime() &&
				new Date(act.date).getTime() <= endDate.getTime(),
		)

		const scheduled = activitiesJson.filter((ac) => ac.scheduled === 'scheduled')
		const rescheduled = activitiesJson.filter(
			(ac) => ac.rescheduled === 'rescheduled',
		)
		const finished = activitiesJson.filter((ac) => ac.scheduled === 'finished')
	...
	async indexByPayment({ request, }) {
		...
		const payments = await PaymentActivity.where({
			...
			.where('created_at', '>=', subHours(startDate, 3))
	...
	async indexByPaymentType({ request, }) {
		// PaymentActivity
		const userLogged = request.qs
		const data = request.only(['date_start', 'date_end'])
		...
		const paymentsJson = payments.toJSON()
		const paymentsCorrect = paymentsJson.filter(
			(v) => v.type_payment && v.type_payment.value,
		)
	...
	async indexByPaymentProf({ request, }) {
		// PaymentActivity
		const userLogged = request.qs
		const data = request.only(['date_start', 'date_end'])
	...
	async indexByPaymentProfDate({ request, , response }) {
		// PaymentActivity
		const userLogged = request.qs
		const data = request.only(['date_start', 'date_end'])
		...
		const activitiesJson = activities
			.toJSON()
			.filter((act) => act.scheduled !== 'not_count')
		const procedures = activitiesJson.map((act) =>
			act.procedures.map((proc) => ({
				...proc,
				date: act.updated_at,
				activity: act,
			})),
	...
	async indexByPaymentProcedure({ request, }) {
		// PaymentActivity
		const userLogged = request.qs
		const data = request.only(['date_start', 'date_end'])
