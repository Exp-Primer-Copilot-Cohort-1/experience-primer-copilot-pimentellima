...
export class FindAllHealthInsuranceUseCase implements UseCase<OptsQueryDefault, any[]> {
	constructor(
		private readonly findAllByName: UseCase<OptsQueryDefault, any[]>,
		private readonly findAllByUnity: UseCase<OptsQueryDefault, any[]>,
	) { }

	public async execute(opts: OptsQueryDefault): PromiseEither<AbstractError, any[]> {
		if (opts?.name) {
