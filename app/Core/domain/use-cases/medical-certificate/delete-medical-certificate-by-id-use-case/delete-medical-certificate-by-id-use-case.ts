...
import {
	MedicalCertificateManagerInterface
...
export class DeleteMedicalCertificateByIdUseCase implements UseCase<Input, IMedicalCertificate> {
	...
	public async execute(input: Input): PromiseEither<AbstractError, IMedicalCertificate> {
		...
		const medicalCertificateErr = await this.medicalCertificateManager.deleteMedicalCertificateById(
			input.id,
		)
