...
export class FindAllProceduresByUnityUseCase implements UseCase<FindAllProps, any[]> {
	...
	public async execute(input: FindAllProps): PromiseEither<AbstractError, any[]> {
		...
		const proceduresOrErr = await this.proceduresManager.findById(input.unity_id)
		if (proceduresOrErr.isLeft()) {
			return left(proceduresOrErr.extract())
		}
		return proceduresOrErr
