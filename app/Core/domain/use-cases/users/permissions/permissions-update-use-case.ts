...
type In = { id: string } & Partial<IPermission>

@injectable()
@registry([{ token: PermissionsUpdateUseCase, useClass: PermissionsUpdateUseCase }])
	...
	implements UseCase<In, IPermission>
{
	...
	public async execute({ id, ...permissions }: In): PromiseEither<AbstractError, IPermission> {
		if (!id) return left(new IdNotProvidedError())

		return await this.manager.update(id, permissions)
	}
}
