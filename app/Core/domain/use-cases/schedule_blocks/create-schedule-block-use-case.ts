	...
	implements UseCase<Props, IScheduleBlock>
{
	...
	public async execute(
		...
		const newScheduleBlockOrErr =
			await this.scheduleBlockManager.createScheduleBlock(
				params.unity_id,
				params
			);

		if (newScheduleBlockOrErr.isLeft())
			return left(newScheduleBlockOrErr.extract());
