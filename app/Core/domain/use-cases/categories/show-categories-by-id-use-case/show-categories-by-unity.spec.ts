...
import { ShowCategoriesByIdUseCase } from './show-categories-by-unity-use-case'

const makeSut = () => {
	const sut = new ShowCategoriesByIdUseCase(new CategoriesInMemoryRepository())

	return { sut }
}

describe('ShowCategoriesByIdUseCase (Unit)', () => {
	...
	it('should return UnitNotFoundError if left', async () => {
		const { sut } = makeSut()
		const categoriesOrErr = await sut.execute({ id: 'id_inexistente' })
