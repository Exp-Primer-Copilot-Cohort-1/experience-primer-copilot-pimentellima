...
describe("Update Answer (Unit)", () => {
    it("should update answer", async () => {
        ...
        repo.answers = [oldAnswer];
        const respOrErr = await new UpdateAnswerByIdUseCase(repo).execute({
            answer: {
                ...oldAnswer,
                active: false
            },
    ...
    it("should throw missing id error", async () => {
        const repo = new AnswerInMemoryRepository();
        repo.answers = [{ ...answer, active: false }];
        const respOrErr = await new UpdateAnswerByIdUseCase(repo).execute(
            answer as any
        );
