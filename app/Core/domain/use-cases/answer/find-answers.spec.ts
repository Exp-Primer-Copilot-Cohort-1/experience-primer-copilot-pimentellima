...
describe("Find all answers (Unit)", () => {
	it("should find all answers that matches unity_id", async () => {
		...
		const respOrErr = await new FindAllAnswersUseCase(repo).execute({
			unity_id: "1",
		});
	...
	it("should throw missing unity_id param error", async () => {
		const respOrErr = await new FindAllAnswersUseCase(
			new AnswerInMemoryRepository()
		).execute({} as any);
