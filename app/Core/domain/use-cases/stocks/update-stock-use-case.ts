...
@injectable()
@registry([{ token: UpdateStockUseCase, useClass: UpdateStockUseCase }])
export class UpdateStockUseCase implements UseCase<IStock, IStock> {
	...
	public async execute(
		...
		const stock = await Stock.findByIdAndUpdate(id, parsedData, {
			new: true,
		}).orFail()

		return right(stock.toObject())
	}
}
