...
export class FormMongoRepository implements FormManagerInterface {
    ...
    async findFormsByProfId (unity_id: string, prof_id: string) : PromiseEither<AbstractError, FormEntity[]> {
		if (!unity_id) return left(new MissingParamsError("unity id"));
    ...
    async findFormsByCategoryId (unity_id: string, category_id: string) : PromiseEither<AbstractError, FormEntity[]> {
		if (!unity_id) return left(new MissingParamsError("unity id"));
