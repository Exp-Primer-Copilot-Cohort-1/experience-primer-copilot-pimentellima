...
export class OptsQuery implements IOptsQuery {
	...
	defineSort(sort: string = '--name'): OptsQuery {
		this._sort = sort;
		return this;
	}

	defineSize(limit: number = 100): OptsQuery {
		this._limit = Number(limit);
		return this;
	}

	defineSkip(skip: number = 0): OptsQuery {
		this._skip = Number(skip);
		return this;
	}

	defineActive(active: boolean = true): OptsQuery {
